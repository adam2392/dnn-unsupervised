...
def maintrain(dnnmodel, datamunger, outputdatadir, tempdatadir):
    ...
    trainer = traincnnaux.TrainFragAux(dnnmodel, batch_size, NUM_EPOCHS, AUGMENT)
...
if __name__ == '__main__':
    ...
    outputdatadir = str(sys.argv[1])            # the output data directory - where to store the final model, weights
    tempdatadir = str(sys.argv[2])              # the temporary data directory - storing the computations at intervals
    traindatadir = str(sys.argv[3])             # the training data directory - storing fragility maps
    rawdatadir = str(sys.argv[4])               # the datadir where the original raw data is
