...
class iEEGSeq(BaseNet):
    ...
    def buildmodel(self, convnet):   
        ...
        if self.name == 'SAMECNNLSTM':
            ...
            self._build_cnn_lstm_mix(convnet, num_timewins=num_timewins, size_mem=size_mem, BIDIRECT=self.BIDIRECT)
        elif self.name == 'LOAD':
            self._appendlstm(convnet, num_timewins=num_timewins, size_mem=size_mem)
    ...
    def _appendlstm(self, fixed_model, num_timewins, size_mem, BIDIRECT):
        # create sequential model to get this all before the LSTM
        self.model.add(TimeDistributed(fixed_model, input_shape=(num_timewins,)+self.input_shape))
        if BIDIRECT:
            self.model.add(Bidirectional(LSTM(units=size_mem, 
                            activation='relu', 
                            return_sequences=False)))
