...
class Postalarm(object):
    ...
    def temporal_smooth(self, thresh=0.5):
        ...
        for idx, win in enumerate(self.samplepoints):
            ...
            curr_fpower = np.sum(self.seqoutputs[lowend:highend]) / self.winsamps
            
            # once we've passed a window, we can raise another alarm if we find it
            if alarm_inds:
    ...
    def compute_timepoints(self, winsize_persamp, stepsize_persamp, numtimepoints, copy=True):
        ...
        timestarts = np.arange(0, numtimepoints - winsize_persamp + 1, stepsize_persamp)
        timeends = np.arange(winsize_persamp - 1, numtimepoints, stepsize_persamp)
