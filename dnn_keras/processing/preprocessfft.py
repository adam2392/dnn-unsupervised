...
class PreProcess(object):
    ...
    def _computefreqindices(self, freqs, freqbands):
        ...
        for band in freqbands:
            ...
            freqbandindices[band] = np.where(
                (freqs >= lowerband) & (freqs < upperband))
            freqbandindices[band] = [freqbandindices[band]
                                     [0][0], freqbandindices[band][0][-1]]
    ...
    def binFrequencyValues(self, power, freqs):
        ...
        for idx, band in enumerate(freqbandindices):
            ...
            power_binned[:, idx, :] = np.mean(
                np.abs(power[:, indices[0]:indices[1]+1, :]), axis=1)  # [np.newaxis,:,:]
    ...
    def compresspowermat(self, datapath):
        ...
        for idx, band in enumerate(freqbandindices):
            indices = freqbandindices[band]
            # average between these two indices
            powerbands[band] = np.mean(
                power[:, indices[0]:indices[1]+1, :], axis=1)  # [np.newaxis,:,:]
    ...
    def projectpower_gain(self, powerbands, metadata, verts, areas, regmap):
        ...
        for idx, band in enumerate(powerbands):
            mapped_power_band = np.tensordot(
                modgain, powerbands[band], axes=([1], [0]))
    ...
    def projectpower_knn(self, powerbands, metadata):
        ...
        for ichan in range(0, len(seeg_labels)):
            ...
            for idx, band in enumerate(powerbands):
                ...
                mapped_power_bands[near_indices, idx,
                                   :] += chanpower.astype('float64')
    ...
    def projectpower_invsquare(self, powerbands, metadata):
        ...
        # computes distance along each axis
        dr = region_centers - seeg_xyz[:, np.newaxis]
        ...
        Vr[small_indices] = np.nanmax(
            np.ma.masked_invalid(Vr[:])) + np.nanmin(Vr[:])
